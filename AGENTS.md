...
- `pkg/seamcarve/seamcarve.go` — content-aware aspect fitting. `Cover(src, dstW, dstH)` fills the target EXACTLY: when the page aspect matches the target (within 1%) it does pure high-quality uniform interpolation (`CatmullRom`); otherwise it scales uniformly to COVER the target (never crops/stretches/bars) and removes the overflow with seam carving. `Cover` auto-builds a structure mask (gradient threshold + dilation) so long diagonal objects (a sickle blade, lettering) are protected and rescaled uniformly via the hybrid path instead of being sheared by seams. It never inserts seams or adjusts levels (preserves original color/contrast). `ResizeWithMask` takes an explicit soft mask; `UseForwardEnergy`/`SetLevels`/`SetCenterShield` tune the energy.
...
- Upscaling logic: when a page is smaller than the target resolution (`width > srcW || height > srcH`), run Real-ESRGAN 4x (`esr.Upscale`) then `seamcarve.Cover`; otherwise `seamcarve.Cover` the source directly. `Cover(src, w, h)` fills `w x h` exactly (uniform interpolation when aspect matches, otherwise cover-scale + seam carving of the overflow). Upscale tiles use an effective 64x64 area centered in the 128x128 model input with 32px padding, discarding the padded output borders.
