...
- Upscaling logic: when a page is smaller than the target resolution (`width > srcW || height > srcH`), run Real-ESRGAN 4x (`esr.Upscale`) then seam carving to exact dims; otherwise seam carve the source directly. `seamcarve.Resize` is pure Go (forward/backward energy, batched seam reduction, stepped expansion). It applies a **center-weighted energy shield** (protects the center from being crushed; active in BOTH backward `getEnergy` and forward `getForwardSeam` via `centerWeight`, `SetCenterShield` to tune/disable) and a **levels contrast filter** at the end (restores deep blacks / pure whites; `SetLevels` to tune/disable). `ResizeWithMask(src, mask, w, h)` takes an optional B&W protection mask where **white = protected** (additive `maskPenalty`, so even zero-energy uniform pixels can't be cut); the mask plane is threaded through remove/insert/transpose in sync with color. `fitToTarget` pre-scales to ~1.2x the target so seam carving only adjusts the residual (avoiding pathological slowdown when 4x or source far exceeds the target). Upscale tiles use an effective 64x64 area centered in the 128x128 model input with 32px padding, discarding the padded output borders.
