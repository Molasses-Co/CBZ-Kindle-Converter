...
- `pkg/retarget/retarget.go` — target-resolution fitting (replaces the removed seamcarve): same aspect → uniform scale; different aspect → uniform scale to **cover** + saliency-aware center crop (`cropWindow`). `Fit(src, mask, dstW, dstH)` takes the optional B&W protection mask (white = protected) to bias the crop window toward the salient region.
...
- Upscaling logic: when a page is smaller than the target resolution (`width > srcW || height > srcH`), run Real-ESRGAN 4x (`esr.Upscale`) then `retarget.Fit` to exact dims; otherwise `retarget.Fit` the source directly. `retarget.Fit(src, mask, w, h)` preserves the aspect via uniform scale and, for a different aspect, cover-crops with a saliency-aware window biased by the protection mask. Upscale tiles use an effective 64x64 area centered in the 128x128 model input with 32px padding, discarding the padded output borders.
