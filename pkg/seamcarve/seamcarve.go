...
// Estrutura: limiar da magnitude do gradiente (Scharr, valores 0-255) acima do
// qual um pixel é marcado como protegido, e o número de passadas de dilatação
// 3x3 para conectar line art em componentes fluentes.
const (
	structureThreshold float64 = 40
	structureDilate    int     = 2
)
...
// Cover redimensiona src para preencher exatamente dstW x dstH, preservando
// geometricamente o conteúdo. Quando o aspect ratio coincide com o alvo,
// aplica apenas interpolação uniforme de alta qualidade (sem crop/carving).
// Caso contrário, escala uniformemente para COBRIR o alvo (sempre >= dstW/dstH
// em ambos os eixos, jamais estica/encaixa com sobras) e remove o excedente
// com seam carving, que corta colunas/linhas de menor energia (fundo plano,
// vinhetas, gutters) preservando faces, linha e texto. Nunca insere seam nem
// aplica ajuste de níveis (preserva a cor e o contraste originais).
func Cover(src image.Image, dstW, dstH int) image.Image {
	return CoverWithMask(src, nil, dstW, dstH)
}

// CoverWithMask é como Cover, mas aceita uma máscara suave de proteção (branco =
// proteger) para guiar o seam carving longe do conteúdo saliente.
func CoverWithMask(src image.Image, mask image.Image, dstW, dstH int) image.Image {
	...
	// Aspecto igual (tolerância 1%): interpolação uniforme, nada a remover.
	if sameAspect(srcW, srcH, dstW, dstH) {
		return resize(src, dstW, dstH)
	}

	// Escala uniforme para COBRIR o alvo: o maior fator garante que ambos os
	// eixos fiquem >= ao tamanho alvo, então o carving só remove (nunca insere),
	// e o resultado é exatamente dstW x dstH após a remoção do excedente.
	scale := math.Max(float64(dstW)/float64(srcW), float64(dstH)/float64(srcH))
	cw := int(math.Round(float64(srcW) * scale))
	ch := int(math.Round(float64(srcH) * scale))
	if cw < dstW {
		cw = dstW
	}
	if ch < dstH {
	...
	covered := resize(src, cw, ch)

	if mask == nil {
		mask = structureMaskImage(covered)
	}
	p := newPlanes(covered, mask)
	if p.w == 0 || p.h == 0 {
		return covered
	}
	if comps := protectedComponents(p.protect, p.w, p.h, protectCrossThr); len(comps) > 0 {
		return hybridResizeLevels(p, covered, dstW, dstH, comps, false)
	}
	carveExact(p, dstW, dstH)
	return p.image()
}

// structureMaskImage gera uma máscara binária de proteção a partir de uma
// imagem de origem, marcando como protegidos (branco) os pixels com gradiente de
// estrutura forte (line art / bordas), seguido por uma dilatação que conecta as
// linhas próximas para que o flood-fill enxergue objetos conectados (ex.: uma
// lâmina de foice). Passar esta máscara faz o Cover ativar o hybridResize, que
// redimensiona os objetos longos de forma uniforme em vez de cortá-los com seams
// (que distorce elementos diagonais).
func structureMaskImage(src image.Image) image.Image {
	b := src.Bounds()
	w, h := b.Dx(), b.Dy()
	if w == 0 || h == 0 {
		return nil
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			gx, gy := scharrAt(gray, w, h, x, y)
			mag := float64(abs32(gx) + abs32(gy))
			if mag >= structureThreshold {
				prot[y*w+x] = true
			}
			sum += mag
		}
	...
	for pass := 0; pass < structureDilate; pass++ {
		next := make([]bool, w*h)
		for y := 0; y < h; y++ {
			for x := 0; x < w; x++ {
				if prot[y*w+x] {
...
func carvePlanes(p *planes, width, height int) image.Image {
	return carvePlanesLevels(p, width, height, levelsEnabled)
}

// carvePlanesLevels like carvePlanes but only applies the levels filter when
// applyLevels is true. Cover needs the false path so it preserves the original
// contrast/colors (Cover never adjusts levels).
func carvePlanesLevels(p *planes, width, height int, doLevels bool) image.Image {
	p.resizeWidth(width)
	p.transpose()
...
// carveExact redimensiona para width x height SEM aplicar níveis (preserva a
// cor/contraste originais) e apenas removendo (nunca inserindo) seams.
func carveExact(p *planes, width, height int) {
	if width < p.w {
		p.resizeWidth(width)
	}
	p.transpose()
...
func hybridResize(p *planes, src image.Image, width, height int, comps []component) image.Image {
	return hybridResizeLevels(p, src, width, height, comps, levelsEnabled)
}

// hybridResizeLevels like hybridResize but only applies the levels filter when
// applyLevels is true. Cover needs the false path to preserve original tones.
func hybridResizeLevels(p *planes, src image.Image, width, height int, comps []component, doLevels bool) image.Image {
	srcW, srcH := p.w, p.h
	bg := carvePlanesLevels(p.backgroundPlanes(protectCrossThr), width, height, doLevels).(*image.RGBA)
...
// ---- Helpers locais para Cover (interpolação uniforme de alta qualidade) ----

// sameAspect reporta se w1xh1 e w2xh2 têm o mesmo aspect ratio (tolerância 1%).
...
// toRGBA converte a imagem para *image.RGBA.
